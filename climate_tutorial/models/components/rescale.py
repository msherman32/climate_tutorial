...
class MinMaxRescaleTransformation(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        """
        # Essentially returns: return torch.mul(torch.divide(x - self.r_min, r_diff), t_diff) + self.t_min
        ...
        r_min_shape = len(self.r_min)
        r_min = torch.from_numpy(self.r_min)
        r_min = torch.reshape(r_min, (r_min_shape,1,1)) # all climate variable will come in as (C,x,y)
        x = x - r_min

        r_diff = self.r_max - self.r_min
        r_diff_shape = len(r_diff) # length 3 array
        r_diff = torch.from_numpy(r_diff) # length 3 tensor
        ...
        r_diff = torch.reshape(r_diff, (r_diff_shape,1,1)) 
        # r_diff_one = torch.full(correct_shape, r_diff[0])
        # r_diff_two = torch.full(correct_shape, r_diff[1])
        # r_diff_three = torch.full(correct_shape, r_diff[2])
        ...
        t_diff = self.t_max - self.t_min # scalar 
        t_diff = torch.full(x.shape, t_diff)
        ...
        # t_diff = [t_diff_one, t_diff_one, t_diff_one]
